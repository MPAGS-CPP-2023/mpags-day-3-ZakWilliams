...
int main(int argc, char* argv[])
{
    ...
    ProgramSettings cmd_line{false, false, "", "", "", true}; //FIX INITIALISATION OF CMD_LINE

    // Process command line arguments
    const bool cmdLineStatus{
        processCommandLine(cmdLineArgs, cmd_line)};
    ...
    if (!cmd_line.inputFile.empty()) {
        // Open the file and check that we can read from it
        std::ifstream inputStream{cmd_line.inputFile};
        if (!inputStream.good()) {
            std::cerr << "[error] failed to create istream on file '"
                      << cmd_line.inputFile << "'" << std::endl;
            return 1;
        }
    ...
    if (!cmd_line.outputFile.empty()) {
        ...
        std::ofstream outputStream{cmd_line.outputFile};
        if (!outputStream.good()) {
            std::cerr << "[error] failed to create ostream on file '"
                      << cmd_line.outputFile << "'" << std::endl;
            return 1;
        }
